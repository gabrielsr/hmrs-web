...
from .credential import OauthCredential, PasswordCredential
...
def register_from_oauth(provider:str, oauth_resp_content: str) -> Principal:
    try:
        ...
        profile = Profile(name=name, email=email, principal=principal, picture=picture)
...
def email_already_registered(email):
    exists = db.session.query(Profile.id).filter_by(email=email).first()
